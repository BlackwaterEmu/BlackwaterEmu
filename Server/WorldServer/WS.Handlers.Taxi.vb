...
Public Module WS_Handlers_Taxi
    ...
    End Enum
    Private Sub SendActivateTaxiReply(ByVal Client As ClientClass, ByVal Reply As ActivateTaxiReplies)
    ...
    Public Sub SendTaxiStatus(ByVal c As CharacterObject, ByVal cGUID As ULong)
    ...
    Public Sub On_CMSG_TAXINODE_STATUS_QUERY(ByRef packet As PacketClass, ByRef Client As ClientClass)
        ...
        Dim currentTaxi As Integer = GetNearestTaxi(WORLD_CREATUREs(GUID).positionX, WORLD_CREATUREs(GUID).positionY, WORLD_CREATUREs(GUID).MapID)
        ...
        SMSG_TAXINODE_STATUS.AddUInt64(GUID)
    ...
    Public Sub On_CMSG_TAXIQUERYAVAILABLENODES(ByRef packet As PacketClass, ByRef Client As ClientClass)
        ...
        Dim GUID As ULong = packet.GetUInt64
        If WORLD_CREATUREs.ContainsKey(GUID) = False Then Exit Sub
        If (WORLD_CREATUREs(GUID).CreatureInfo.cNpcFlags And NPCFlags.UNIT_NPC_FLAG_FLIGHTMASTER) = 0 Then Exit Sub 'NPC is not a taxi vendor
        ...
        SMSG_SHOWTAXINODES.AddUInt64(GUID)
    ...
    Public Sub On_CMSG_ACTIVATETAXI(ByRef packet As PacketClass, ByRef Client As ClientClass)
        ...
        If WORLD_CREATUREs.ContainsKey(GUID) = False OrElse (WORLD_CREATUREs(GUID).CreatureInfo.cNpcFlags And NPCFlags.UNIT_NPC_FLAG_FLIGHTMASTER) = 0 Then
        ...
        If Client.Character.Side Then
            ...
            Mount = CREATURESDatabase(TaxiNodes(srcNode).HordeMount).Model
        Else
            ...
            Mount = CREATURESDatabase(TaxiNodes(srcNode).AllianceMount).Model
        End If
        If Mount = 0 Then
    ...
    End Sub
    Public Sub On_CMSG_ACTIVATETAXIEXPRESS(ByRef packet As PacketClass, ByRef Client As ClientClass)
        If (packet.Data.Length - 1) < 21 Then Exit Sub
        packet.GetInt16()
        Try
            Dim GUID As ULong = packet.GetUInt64
            ...
            Log.WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_ACTIVATETAXIEXPRESS [taxiGUID={2:X} TotalCost={3} NodeCount={4}]", Client.IP, Client.Port, GUID, TotalCost, NodeCount)
    ...
    Public Sub TaxiMove(ByVal Character As CharacterObject, ByVal DiscountMod As Single)
        ...
        Try
            ...
            For Each Path As Integer In WaypointPaths
                ...
                WaypointNodes.Clear()
                For Each TaxiPathNode As KeyValuePair(Of Integer, TTaxiPathNode) In TaxiPathNodes
                ...
                For i As Integer = 0 To WaypointNodes.Count - 1
                    ...
                    Next
                    'Character.Client.SendMultiplyPackets(p)
                    Character.SendToNearPlayers(p)
                    p.Dispose()

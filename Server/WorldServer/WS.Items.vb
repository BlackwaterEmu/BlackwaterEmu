...
#Region "WS.Items.Constants"
    ...
    Public Enum EnchantSlots As Byte
        ENCHANTMENT_PERM = 0
        ENCHANTMENT_TEMP = 1
        ENCHANTMENT_SOCKET_START = 2
        ENCHANTMENT_SOCKET_GEM_1 = 2
        ENCHANTMENT_SOCKET_GEM_2 = 3
        ENCHANTMENT_SOCKET_GEM_3 = 4
        ENCHANTMENT_SOCKET_END = 5
        ENCHANTMENT_BONUS = 5
        MAX_INSPECT = 6
        ENCHANTMENT_PROP_SLOT_1 = 6 'used with RandomSuffix
        ENCHANTMENT_PROP_SLOT_2 = 7 'used with RandomSuffix
        ENCHANTMENT_PROP_SLOT_3 = 8 'used with RandomSuffix and RandomProperty
        ENCHANTMENT_PROP_SLOT_4 = 9 'used with RandomProperty
        ENCHANTMENT_PROP_SLOT_5 = 10 'used with RandomProperty
        MAX_ENCHANTS = 11
    End Enum
...
#Region "WS.Items.TypeDef"
    ...
    Public Class ItemObject
        ...
        Public Sub FillAllUpdateFlags(ByRef Update As UpdateClass)
            ...
            Else
                ...
                For Each Enchant As KeyValuePair(Of Byte, TEnchantmentInfo) In Enchantments
                    ...
                    Update.SetUpdateFlag(EItemFields.ITEM_FIELD_ENCHANTMENT_1_1 + Enchant.Key * 3 + 1, Enchant.Value.Duration)
        ...
        Public Function GetGemCount(ByVal GemID As Integer) As Byte
            Dim count As Byte = 0
            For Each Enchant As KeyValuePair(Of Byte, TEnchantmentInfo) In Enchantments
                If SpellItemEnchantments.ContainsKey(Enchant.Value.ID) Then
                    If SpellItemEnchantments(Enchant.Value.ID).GemID = GemID Then count += 1
                End If
            Next
            Return count
        End Function
        ...
        Public Sub RemoveEnchantment(ByVal Slot As Byte)
            ...
            If CHARACTERs.ContainsKey(OwnerGUID) Then
                ...
                Dim tmpUpdate As New UpdateClass(FIELD_MASK_SIZE_ITEM)
                tmpUpdate.SetUpdateFlag(EItemFields.ITEM_FIELD_ENCHANTMENT_1_1 + (Slot * 3), 0)
                tmpUpdate.SetUpdateFlag(EItemFields.ITEM_FIELD_ENCHANTMENT_1_1 + (Slot * 3) + 1, 0)
                tmpUpdate.SetUpdateFlag(EItemFields.ITEM_FIELD_ENCHANTMENT_1_1 + (Slot * 3) + 2, 0)
                tmpUpdate.AddToPacket(packet, ObjectUpdateType.UPDATETYPE_VALUES, Me)
    ...
    Public Class TDamage
        ...
        Public Type As DamageTypes = DamageTypes.DMG_PHYSICAL
    End Class
...
#Region "WS.Items.Handlers"
    ...
    Public Sub On_CMSG_SOCKET_GEMS(ByRef packet As PacketClass, ByRef Client As ClientClass)
        Try
            If (packet.Data.Length - 1) < 37 Then Exit Sub
            packet.GetInt16()
            Dim ItemGUID As ULong = packet.GetUInt64
            Dim GemGUID(2) As ULong
            GemGUID(0) = packet.GetUInt64
            GemGUID(1) = packet.GetUInt64
            GemGUID(2) = packet.GetUInt64
            If ItemGUID = 0 OrElse (GemGUID(0) = 0 AndAlso GemGUID(1) = 0 AndAlso GemGUID(2) = 0) Then Exit Sub
            Log.WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_SOCKET_GEMS [ItemGUID={2:X} Gem1={3:X} Gem2={4:X} Gem3={5:X}]", Client.IP, Client.Port, ItemGUID, GemGUID(0), GemGUID(1), GemGUID(2))

            'DONE: Stop cheat that tries to socket with the same gem in more than one socket
            If (GemGUID(0) AndAlso (GemGUID(0) = GemGUID(1) OrElse GemGUID(0) = GemGUID(2))) OrElse (GemGUID(1) AndAlso (GemGUID(1) = GemGUID(2))) Then Exit Sub

            'DONE: Get the Item
            Dim Item As ItemObject = Client.Character.ItemGETByGUID(ItemGUID)
            If Item Is Nothing Then Exit Sub 'Item not found

            'DONE: Get the Gems + GemProperties
            Dim Gems(2) As ItemObject, GemProps(2) As TGemProperties
            For i As Byte = 0 To 2
                Gems(i) = Client.Character.ItemGETByGUID(GemGUID(i))
                If Gems(i) IsNot Nothing Then
                    If GemProperties.ContainsKey(Gems(i).ItemInfo.GemProperties) Then
                        GemProps(i) = GemProperties(Gems(i).ItemInfo.GemProperties)
                    Else
                        GemProps(i) = Nothing
                    End If
                Else
            ...
            'DONE: Check if the gem fits in the socket
            For i As Byte = 0 To 2
                If GemProps(i) IsNot Nothing AndAlso (Item.ItemInfo.Sockets(i).Color = 0 OrElse _
                (Item.ItemInfo.Sockets(i).Color = ITEM_GEMS.COLOR_META AndAlso GemProps(i).Color <> ITEM_GEMS.COLOR_META) OrElse _
                (Item.ItemInfo.Sockets(i).Color <> ITEM_GEMS.COLOR_META AndAlso GemProps(i).Color = ITEM_GEMS.COLOR_META)) Then
                    Exit Sub
                End If
            Next

            'DONE: Different checks for unique-equipped
            For i As Byte = 0 To 2
                If Gems(i) IsNot Nothing AndAlso (Gems(i).ItemInfo.Flags And ITEM_FLAGS.ITEM_FLAGS_UNIQUE_EQUIPPED) Then
                    'DONE: Check if this gem is already equipped if it's unique equipped
                    If Item.IsEquipped AndAlso Client.Character.ItemGETWithGem(Gems(i).ItemEntry) IsNot Nothing Then
                        SendInventoryChangeFailure(Client.Character, InventoryChangeFailure.EQUIP_ERR_ITEM_UNIQUE_EQUIPABLE, ItemGUID, 0)
                        Exit Sub
                    End If

                    'DONE: Check if the character is trying to equip two of the same unique equipped gems in the same item
                    For j As Byte = 0 To 2
                        If i <> j AndAlso Gems(j) IsNot Nothing AndAlso Gems(i).ItemEntry = Gems(j).ItemEntry Then
                            SendInventoryChangeFailure(Client.Character, InventoryChangeFailure.EQUIP_ERR_ITEM_UNIQUE_EQUIPPABLE_SOCKETED, ItemGUID, 0)
                            Exit Sub
                        End If
                    Next
                End If
            ...
            'DONE: Add the gems as enchantments and remove the gems
            For i As Byte = 0 To 2
                If GemProps(i) IsNot Nothing Then Item.AddEnchantment(GemProps(i).SpellItemEnchantment, i + EnchantSlots.ENCHANTMENT_SOCKET_START)
                If Gems(i) IsNot Nothing Then Client.Character.ItemREMOVE(Gems(i).GUID, True, True)
            Next

            'DONE: Send item update to the client
            Client.Character.SendItemUpdate(Item)

            'TODO: Gem Socket Bonus
            'TODO: Activate meta gems that now fits the requirements
        Catch e As Exception
            Log.WriteLine(LogType.CRITICAL, "Error when socketing item.{0}", vbNewLine & e.ToString)
        End Try
    End Sub
    ...
    Public Sub SetVirtualItemInfo(ByVal c As CharacterObject, ByVal Slot As Byte, ByRef Item As ItemObject)
        If Item Is Nothing Then
            ...
            c.SetUpdateFlag(EUnitFields.UNIT_VIRTUAL_ITEM_SLOT_ID + Slot, 0)
        Else
            ...
            c.SetUpdateFlag(EUnitFields.UNIT_VIRTUAL_ITEM_SLOT_ID + Slot, Item.ItemInfo.Model)
        End If
    End Sub
                    ...
                    For j As Byte = 0 To 2
                        If SPELLs(itemInfo.Spells(i).SpellID).SpellEffects(j) IsNot Nothing AndAlso SPELLs(itemInfo.Spells(i).SpellID).SpellEffects(j).ID = SpellEffects_Names.SPELL_EFFECT_SUMMON Then
                            If SPELLs(itemInfo.Spells(i).SpellID).SpellEffects(j).MiscValueB = SummonType.SUMMON_TYPE_CRITTER OrElse SPELLs(itemInfo.Spells(i).SpellID).SpellEffects(j).MiscValueB = SummonType.SUMMON_TYPE_CRITTER2 Then
                                If Client.Character.NonCombatPet IsNot Nothing Then InstantCast = True
                            End If
                        End If
                    Next
                End If

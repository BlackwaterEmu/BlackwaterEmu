...
Public Module WS_Channels
    ...
    Public Class ChatChannelClass
        ...
        Public Sub New(ByVal Name As String, ByVal Index As Integer)
            ...
            If ChatChannelsInfo.ContainsKey(Index) Then
                'Default channel
                ChannelFlags = ChannelFlags Or CHANNEL_FLAG.CHANNEL_FLAG_GENERAL
                Announce = False
                Moderate = False

                With ChatChannelsInfo(Index)
        ...
        Public Sub Say(ByVal Message As String, ByVal msgLang As Integer, ByRef Character As CharacterObject)
            ...
            End If
        End Sub
        Public Overridable Sub Join(ByRef Character As CharacterObject, ByVal ClientPassword As String, ByVal Index As Integer)
        ...
        Protected Enum CHANNEL_USER_FLAG As Byte
            ...
            CHANNEL_FLAG_VOICED = &H4
            CHANNEL_FLAG_MUTED = &H8
        ...
        Protected Enum CHANNEL_NOTIFY_FLAGS
            ...
            CHANNEL_VOICE_ON = &H22                 ' [%s] Channel voice enabled by %s.
            CHANNEL_VOICE_OFF = &H23                ' [%s] Channel voice disabled by %s.
    ...
    Public Class VoiceChatChannelClass
        ...
        Public VoiceEnabled As Boolean = True

        Public Sub New(ByVal Name As String, ByVal Index As Integer)
            MyBase.New(Name, Index)

            'TODO: Get voice channel
            VoiceID = 153
        End Sub
        ...
        Public Overrides Sub Join(ByRef Character As CharacterObject, ByVal ClientPassword As String, ByVal ChannelType As Integer)
            MyBase.Join(Character, ClientPassword, ChannelType)

            VoiceJoin(Character)
            VoiceUpdate(Character)
        End Sub
        Public Overrides Sub Part(ByRef Character As CharacterObject)
            MyBase.Part(Character)

            VoicePart(Character)
        End Sub
        Public Sub SetSilence(ByRef Character As CharacterObject, ByVal Name As String)
            If Not Joined.Contains(Character.GUID) Then
                Dim packet As PacketClass = BuildChannelNotify(CHANNEL_NOTIFY_FLAGS.CHANNEL_NOT_ON, Character.GUID, Nothing, Nothing)
                Character.Client.Send(packet)
                packet.Dispose()
            ...
            Else
                ...
                For Each GUID As ULong In Joined.ToArray
                    If CHARACTERs(GUID).Name.ToUpper = Name.ToUpper Then
                        Dim flags As Byte = Joined_Mode(GUID)
                        Joined_Mode(GUID) = Joined_Mode(GUID) Or CHANNEL_USER_FLAG.CHANNEL_FLAG_MICROPHONE_MUTE

                        Dim response As PacketClass = BuildChannelNotify(CHANNEL_NOTIFY_FLAGS.CHANNEL_MODE_CHANGE, Character.GUID, flags, Nothing)
                        Broadcast(response)
                        response.Dispose()
                        Exit Sub
                    End If
        ...
        Public Sub VoiceEnable(ByRef Character As CharacterObject)
            ...
            Else
                ...
                Dim packet As PacketClass = BuildChannelNotify(CHANNEL_NOTIFY_FLAGS.CHANNEL_VOICE_ON, Character.GUID, Nothing, Nothing)
                Broadcast(packet)
                packet.Dispose()
            End If
        End Sub
        Public Sub VoiceDisable(ByRef Character As CharacterObject)
            If Not Joined.Contains(Character.GUID) Then
                Dim packet As PacketClass = BuildChannelNotify(CHANNEL_NOTIFY_FLAGS.CHANNEL_NOT_ON, Character.GUID, Nothing, Nothing)
                Character.Client.Send(packet)
                packet.Dispose()
            ...
            Else
                ...
                Dim packet As PacketClass = BuildChannelNotify(CHANNEL_NOTIFY_FLAGS.CHANNEL_VOICE_OFF, Character.GUID, Nothing, Nothing)
                Broadcast(packet)
                packet.Dispose()
            End If
        ...
        Public Sub VoiceJoin(ByRef Character As CharacterObject)
            Dim p As New PacketClass(OPCODES.SMSG_AVAILABLE_VOICE_CHANNEL)
            p.AddUInt64(ID)                     'Channel ID
            p.AddInt8(VoiceType)                'Type (00=custom channel, 03=party, 04=raid?)
            p.AddString(ChannelName)            'Name (not applicable to party/raid)
            p.AddUInt64(Character.GUID)         'Player GUID
            Character.Client.Send(p)
            p.Dispose()
        End Sub
        Public Sub VoiceUpdate(ByRef Character As CharacterObject)
            ...
            p.AddUInt64(ID)                             'Channel ID
            p.AddUInt16(VoiceID)                        'Voice Channel ID
            p.AddInt8(VoiceType)                        'Channel Type (0=channel,2=party)
            p.AddString(ChannelName)                    'Channel Name
            ...
            p.AddInt8(Joined.Count)                     'User Count

            'Add ourself as user #0
            p.AddUInt64(Character.GUID)                 'Char GUID
            p.AddInt8(&HE)                              'Char Flags1
            p.AddInt8(&H6)                              'Char Flags2

            'Add others
            Dim i As Integer = 1
            For Each c As ULong In Joined
                If c <> Character.GUID Then
                    p.AddUInt64(c)                  'User GUID
            ...
            p.Dispose()
        End Sub
        Public Sub VoicePart(ByRef Character As CharacterObject)
            If Character.Client IsNot Nothing Then
                Dim p As New PacketClass(OPCODES.SMSG_VOICE_SESSION_LEAVE)
                p.AddUInt64(Character.GUID)         'Player GUID
                p.AddUInt64(ID)                     'Channel ID
                Character.Client.Send(p)
                p.Dispose()

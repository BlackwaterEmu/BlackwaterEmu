        ...
        Public Sub NewLeader(Optional ByVal Leaver As CharacterObject = Nothing)
            ...
            For i As Byte = 0 To Members.Length - 1
                If Members(i) IsNot Nothing AndAlso Members(i).Client IsNot Nothing Then
                    ...
                    Else
                        If (Members(i).GroupFlags And GroupFlags.ASSISTANT) AndAlso ChosenMember = 255 Then
                            ChosenMember = i
        ...
        Public Sub SendGroupList()
            ...
            For i As Byte = 0 To Members.Length - 1
                If Not Members(i) Is Nothing Then
                    ...
                    packet.AddInt8(0)
                    packet.AddInt8(i \ GROUP_SUBGROUPSIZE)                  'CharFlags [SubGroupID + (RaidAssistant=&H80)]
                    packet.AddInt8(Members(i).GroupFlags)                   'AssistantLeader=1, MainTank=2, MainAssist=4
                    packet.AddInt32(&HFFFFFFFE)
                    packet.AddInt32(&H50000000)
                    ...
                    For j As Byte = 0 To Members.Length - 1
                        If (Not Members(j) Is Nothing) AndAlso (Not Members(j) Is Members(i)) Then
                            ...
                            End If
                            packet.AddInt8(j \ GROUP_SUBGROUPSIZE)          'CharFlags [SubGroupID + (RaidAssistant=&H80)]
                            packet.AddInt8(Members(j).GroupFlags)           'AssistantLeader=1, MainTank=2, MainAssist=4
                        End If
                    Next
                    ...
                    packet.AddInt8(DungeonDifficulty)                       'DungeonDifficulty

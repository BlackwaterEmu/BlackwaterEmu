...
#Region "RS_OPCODES"
    Public Sub On_RS_LOGON_CHALLENGE(ByRef data() As Byte, ByRef Client As ClientClass)
        ...
        If ((ClientBuild) >= REQUIRED_BUILD_LOW) And ((ClientBuild) <= REQUIRED_BUILD_HIGH) Then
            ...
            End Select
            Dim data_response_error(3) As Byte
            data_response_error(0) = CMD_AUTH_LOGON_PROOF
            data_response_error(1) = acc_state
            data_response_error(2) = 3
            data_response_error(3) = 0
    ...
    Public Sub On_RS_LOGON_PROOF(ByRef data() As Byte, ByRef Client As ClientClass)
        ...
        Next
        'Check CRC=ClientCRC
        Dim pass_crc As Boolean = True
        'For i = 0 To 19
        'If CRC_Hash(i) <> Client.AuthEngine.CrcHash(i) Then
        'pass_crc = False
        ' Exit For
        ' End If
        'Next
        ...
        If pass_check Then
            ...
            Else
                'Incorrect game file crc
                Console.WriteLine("[{0}] [{1}:{2}] Incorrect crc for user {3}.", Format(TimeOfDay, "hh:mm:ss"), Client.IP, Client.Port, Client.Account)
                Dim data_response(3) As Byte
                data_response(0) = CMD_AUTH_LOGON_PROOF
                data_response(1) = AccountState.LOGIN_BADVERSION
                data_response(2) = 3
                data_response(3) = 0
    ...
    Public Sub On_RS_REALMLIST(ByRef data() As Byte, ByRef Client As ClientClass)
        ...
        For Each Host As System.Data.DataRow In result.Rows
            ...
            Converter.ToBytes(CType(6, Byte), data_response, tmp)
        Next

        '(byte) unknown1, 0x2A
        '(byte) unknown2, 0x00
        data_response(tmp) = &H15
